...
Run the regression checks with `python -m unittest test_catalog test_atlas test_atlas_metadata test_instance_maps -v` and `node --test test_search_worker.cjs test_coverage_controls.cjs test_atlas_roles.cjs`. The existing workflow runs these checks plus full catalog/atlas validation before publishing. Parser cache revisions are explicit, so atlas/layout changes do not invalidate unaffected records; bump the relevant parser revision when changing parsing or record identity.
