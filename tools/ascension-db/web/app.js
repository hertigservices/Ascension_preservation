...
async function detail(key) {
  ...
  notice("");
  const [id, title, kind, mode, source, data, attribution] = r;
  const attributionHtml = attribution ? `<dt>Realm</dt><dd>${esc(attribution.realms?.join(", ") || "Not recorded")}</dd><dt>Attribution</dt><dd>${esc(attribution.status)}${attribution.evidence?.length ? " · " + esc([...new Set(attribution.evidence.map(e => e.basis + " (" + e.field + ")"))].join("; ")) : ""}</dd>` : "";
  let refs = [];
