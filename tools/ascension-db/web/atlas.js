...
export async function showAtlas({root, manifest, gz, notice, esc, params}) {
  ...
  function populateZones() {
    ...
    $('#atlas-zone').innerHTML=items.map(z=>option(z.key,`${zoneTitle(z)} · ${z.count} observations`,zone.key)).join('') || '<option value="">No matching zones</option>';
  }
  ...
  $('#atlas-world').onchange=()=>{$('#atlas-zone-query').value='';populateZones();const preferred=zones.find(z=>z.map===$('#atlas-world').value&&z.count);if(preferred)$('#atlas-zone').value=preferred.key;openZone();};
  ...
  $('#atlas-zone-summary').innerHTML=`<span>${esc(zone.map_name || CONTINENT_NAMES[zone.map] || 'Preserved world')} ${zone.map ? '· map '+esc(zone.map):''}</span><span>${zone.wma ? 'Captured map space '+esc(zone.wma) : 'Source-reported area'}</span><span>${zone.count.toLocaleString()} observations</span>${zone.inventory?.above_stock_range==='1'?'<span class="atlas-custom">Extended map ID</span>':''}`;
  ...
  if(!zone.count){
    ...
    svg.setAttribute('tabindex','-1');svg.setAttribute('aria-label','Map space with no preserved coordinate observations');
    $('.atlas-sidebar').insertAdjacentHTML('beforeend',`<section class="atlas-empty-card"><p class="eyebrow">PRESERVED MAP</p><h3>${esc(zone.map_name || zone.name)}</h3><p>No coordinate observations have been preserved for this map space yet. Its identity and available source records remain part of the atlas.</p>${(worldInfo?.records||[]).map(k=>`<p><a href="#record=${encodeURIComponent(k)}">Open map source record →</a></p>`).join('')}${worldInfo?.areas?.length?`<h4>Named areas</h4><ul>${worldInfo.areas.map(a=>`<li>${a.record?`<a href="#record=${encodeURIComponent(a.record)}">${esc(a.name)}</a>`:esc(a.name)}</li>`).join('')}</ul>`:''}</section>`);
  }
  const img=$('#atlas-image');
  function gridFallback() {if(disposed||job!==epoch)return;mapArt=false;img.removeAttribute('href');$('#atlas-space-label').textContent=zone.space==='inventory'?'Map inventory · no locations preserved':zone.space==='world'?'Area-local schematic · reported raw coordinates':['reported','world'].includes(zone.space)?'Reported coordinates · unverified space':'Coordinate map · artwork unavailable';$('#atlas-art-credit').textContent='Captured bounds / reported coordinates';}
  if(zone.image && /^https:\/\/wow\.zamimg\.com\/images\/wow\/(classic|wotlk)\/maps\/enus\/original\/\d+\.jpg$/.test(zone.image)) {
    img.setAttribute('href',zone.image);img.addEventListener('error',gridFallback,{once:true});$('#atlas-space-label').textContent='Zone percentages · reference artwork';$('#atlas-art-credit').innerHTML=`<a href="${esc(zone.image)}" target="_blank" rel="noopener noreferrer">Reference map hosted by Wowhead</a>`;
  } else gridFallback();
  if(['reported','world','inventory'].includes(zone.space)) $('.atlas-compass').hidden=true;
  ...
  function draw() {
    ...
    svg.setAttribute('viewBox',`${cx-w/2} ${cy-h/2} ${w} ${h}`);$('#atlas-zoom-level').textContent=zoom.toFixed(1)+'×';$('#atlas-out').disabled=!zone.count||zoom<=1;$('#atlas-in').disabled=!zone.count||zoom>=12;$('#atlas-reset').disabled=!zone.count;
    ...
    for(const ps of groups.values()) {
      ...
      const multiple=ps.length>1,description=multiple?`${ps.length} observations. Zoom to inspect.`:`${p.name}, ${LAYERS[visibleRole(p)]?.[0] || visibleRole(p)}, ${p.x.toFixed(1)}, ${p.y.toFixed(1)}`;
      ...
      const activate=()=>{if(multiple&&zoom<12){cx=x;cy=y;zoom=Math.min(12,zoom*2);draw();renderList();url();}else{select(p);if(multiple){$('#atlas-selected').insertAdjacentHTML('beforeend',`<p>${ps.length} observations overlap here.</p><div class="atlas-overlaps">${ps.map(v=>`<button type="button" data-overlap="${esc(v.key)}" class="quiet-button">${esc(v.name)}</button>`).join('')}</div>`);$('#atlas-selected').querySelectorAll('[data-overlap]').forEach(b=>b.onclick=()=>select(ps.find(v=>v.key===b.dataset.overlap)));}}};
    ...
    const empty=$('#atlas-empty-overlay');empty.hidden=visible.length>0;
    empty.textContent=!zone.count?'No mapped observations preserved for this space. Browse its map sources and named areas below.':!filtered.length?'No observations match these filters. Clear filters or enable more layers.':'No observations in this view. Reset the map to see this zone.';
  }
  function select(p,center=false) {
    selected=p.key;
    if(center){cx=p.plot_x*10;cy=p.plot_y*6.67;zoom=Math.max(3,zoom);}
    $('#atlas-selected').innerHTML=`<p class="eyebrow">${esc(LAYERS[visibleRole(p)]?.[0]||visibleRole(p))}</p><h3>${esc(p.name)}</h3><p class="atlas-coordinate">${p.x.toFixed(2)}, ${p.y.toFixed(2)} <small>${['reported','world'].includes(zone.space)?'reported units':'zone %'}</small></p><p>${esc(p.meta.meaning)}</p><dl><dt>Source</dt><dd>${esc(p.source)}</dd><dt>Game mode</dt><dd>${esc(p.mode)}</dd>${Object.entries(p.meta).filter(([k,v])=>k!=='meaning'&&v!==''&&v!==undefined).map(([k,v])=>`<dt>${esc(k.replaceAll('_',' '))}</dt><dd>${esc(v)}</dd>`).join('')}</dl><p><a href="#record=${encodeURIComponent(p.record)}">Open preserved record →</a></p><p><a href="#search?q=${encodeURIComponent(p.id)}&kind=${encodeURIComponent(p.entity)}">Find ${esc(p.entity)} #${esc(p.id)} across sources →</a></p>`;
    draw();url();
  }
  function renderList() {
    page=clamp(page,0,Math.max(0,Math.ceil(filtered.length/30)-1));const rows=filtered.slice(page*30,page*30+30);
    $('#atlas-list').innerHTML=rows.map(p=>`<button class="atlas-list-item layer-${visibleRole(p)}" data-point="${esc(p.key)}"><span class="atlas-list-glyph" aria-hidden="true">${esc(LAYERS[visibleRole(p)]?.[1]||'●')}</span><span><strong>${esc(p.name)}</strong><small>${esc(LAYERS[visibleRole(p)]?.[0]||visibleRole(p))} · ${esc(p.source)}</small></span><span class="atlas-list-coordinate">${p.x.toFixed(1)}<br>${p.y.toFixed(1)}</span></button>`).join('')||'<p class="muted atlas-no-results">No matching observations in this map space.</p>';
  ...
  function filter() {
    ...
    if(selected&&!filtered.some(p=>p.key===selected)){selected='';$('#atlas-selected').innerHTML='<p>Select a marker or an observation to inspect its evidence.</p>';}
  ...
  function zoomBy(f){if(!zone.count)return;zoom=clamp(zoom*f,1,12);draw();url();}
  ...
  svg.addEventListener('keydown',e=>{if(e.target!==svg||!zone.count)return;const delta=60/zoom;if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','=','-','Home'].includes(e.key))e.preventDefault();if(e.key==='ArrowLeft')cx-=delta;if(e.key==='ArrowRight')cx+=delta;if(e.key==='ArrowUp')cy-=delta;if(e.key==='ArrowDown')cy+=delta;if(e.key==='+'||e.key==='=')zoom=clamp(zoom*1.5,1,12);if(e.key==='-')zoom=clamp(zoom/1.5,1,12);if(e.key==='Home'){zoom=1;cx=500;cy=333.5;}draw();url();});
  let drag=null, moved=false;const pointers=new Map();let pinch=0;
  svg.addEventListener('pointerdown',e=>{if(e.target.closest('.atlas-marker')||!zone.count)return;svg.setPointerCapture(e.pointerId);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});drag={x:e.clientX,y:e.clientY,cx,cy};moved=false;if(pointers.size===2){const [a,b]=[...pointers.values()];pinch=Math.hypot(a.x-b.x,a.y-b.y);}});
