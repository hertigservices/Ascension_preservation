...
A final live-run Sol review also caught parser-revision refresh replacing 63 established ledger group labels with the first path rediscovered for identical bytes. Refresh now preserves the existing nonambiguous group and its complete source-label list; the existing one-way ambiguous-to-specific upgrade remains allowed. The 63 incidental labels were identified for an atomic, field-only restoration against the paused snapshot; merged `(hash, group)` placements and payloads were unchanged.
