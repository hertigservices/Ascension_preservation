...
def deliver(snapshot,checkout,push=False,branch='main'):
    snapshot=Path(snapshot).resolve();checkout=Path(checkout).resolve()
    manifest=intake.verify_export(snapshot)
    ...
    with intake.Lock(gd/'research-publication.lock'):
        ...
        target=checkout/rel
        if target.exists():
            current=intake.verify_export(target)
            ...
            intake.verify_export(pending);pending.rename(target)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--snapshot',type=Path);ap.add_argument('--checkout',type=Path,required=True);ap.add_argument('--push',action='store_true');ap.add_argument('--branch',default='main');ap.add_argument('--initialize-checkout',action='store_true');a=ap.parse_args()
    ...
    elif a.snapshot:print(json.dumps(deliver(a.snapshot,a.checkout,a.push,a.branch),indent=2))
