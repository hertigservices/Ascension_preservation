...
| P7 remaining systems | **CoA tabs DONE 2026-09-10.** The Vanity and Wardrobe tabs (the only extra Collections tabs a CoA character sees) open an Ascension-framed panel that states honestly that no vanity / transmog catalogue was recovered (0-byte JSON in every capture); zero errors. Skill Cards, Mystic Enchants and Hero Architect tabs are Hero-mode only. | Tooltip macro post-processing (`@s:…@`, `@re:…@` render raw); wiring the original Vanity/Wardrobe addons if their data is ever found. |
