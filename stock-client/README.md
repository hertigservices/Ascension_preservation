...
| P7 remaining systems | **CoA tabs + tooltips DONE 2026-09-10.** Vanity and Wardrobe show honest placeholders (no catalogue recovered); `compat/zz_TooltipMacros.lua` post-processes Ascension's tooltip macros (`@s:ID:0@` icon+name, `@s:ID:-1@` description, `@ext:...:ext@` behind Hold SHIFT, `@req@`, `@unlockby@`, `@ifknown@`, `@re@`) on GameTooltip/ItemRef/Shopping tooltips, verified against live captures. Skill Cards remain Hero-mode only and unwired. | Skill Cards tab; the original Vanity/Wardrobe addons if their data is ever found (Jealous-Sound's public repack ships three collection DBCs we never had - see the hub's `research/coa-repack/FINDINGS.md`). |
| P8 distribution | **INSTALLER + LAUNCHER DONE 2026-09-10.** `tools/install_client.py` builds `client-coa`; `INSTALL.md` covers client, CoA server and the second Free-Pick realm; the hub's Azeroth Realm launcher now carries the port as its `ascension` profile (Hero realm, replacing the Ascension.exe profile) and `coa-stock` (CoA mode), with process matching that tells two worldservers from one server directory apart by their `-c` config. | A clean-clone rehearsal by someone else (this session's own rehearsal: see HANDOFF §5.10); Linux build of the module; a public release tag; the hub process must be restarted to pick up the launcher fix. |
...
python -B tools/build_addon_pack.py --dataset unattributed-7f0c805382 --class-byte 28 --level 80 --catalogue data/build-catalogue --enchants data/enchants --out build/p2-catalogue-preview
```
...
Review `DESIGN.md` for architecture, `INSTALL.md` for the client and server steps, `server/mod-ascension-ca/README.md` for the wire protocol and rules, and the private hub's `realms/ascension/HANDOFF-STOCK-CLIENT.md` §5 for the per-phase evidence (screenshots, root causes, exact commands). The full port remains the objective. P0-P8 have all reached their acceptance evidence; what remains is verification by someone who is not the author (clean-clone rehearsal of `INSTALL.md`), the Skill Cards tab, and whatever of Jealous-Sound's client-agnostic server work (collection DBCs, the widened spell-effect/aura enum ranges) is worth lifting onto the Free-Pick realm.
